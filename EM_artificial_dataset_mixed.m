...
R = randn(units,units);
U = R.'*(R*R.')^(-1/2);
S = randn(periods,periods);
V = S.'*(S*S.')^(-1/2);

Sigma = zeros(units,periods);
...
serial = zeros(units,periods);
seed = randn(units,20);
serial(1:units,1:20) = seed;

beta = randn(20,1)/5;

for i = 21:periods
    
    serial(:,i) = sum(serial(:,i-20:i-1)*diag(beta),2);

end

%3.Correlation across samples

seed = randn(periods,20);

unital = zeros(periods,units);

unital(1:periods,1:20) = seed;

beta = randn(20,1)/5;

for i = 21:units
    
    unital(:,i) = sum(unital(:,i-20:i-1)*diag(beta),2);
...
%4.
seed = randn(periods,20);
unital_serial = zeros(periods,units);

for i = 21:max(units,periods)
end


%5.Average of the above samples.
mixed = (unital+serial+low_rank)/3;
