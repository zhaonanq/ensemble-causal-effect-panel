...
indices = randi(units-1,num_units,1);
errors = zeros(num_units,3);
...
for i = 1:num_units
    ...
    cv_shift = circshift(MC_cv,[0,indices(i)]);
...
reg_rmse = sqrt(sum(errors.^2,1)/num_units);
...
for left_out = 1:units-1
    ...
    %METHOD-1 MC
    [predicted_outcomes, actual_outcomes, matrix_rank] = MC(cv_shift,1,reg_rate);
    matrix_rank
